...
export class ProductDetail implements OnInit {
  ...
  ngOnInit() {
    ...
    this.productService.getProductById(Number(this.eventId)).subscribe((res: ApiProductoByIdResponse) => {
      this.producto = res.data;
      if(this.producto){
        if(!this.producto.descripcionExtensa){
          this.producto.descripcionExtensa = 'No se tiene información de detalle para este producto.';
        }
      }
    });
  }
